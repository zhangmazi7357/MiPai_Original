...
public class SearchVadieoListFragment extends BaseListFragment<ShopProductsBean.DataBean.InfoBean>{
    String keywords="";
    public static SearchVadieoListFragment newInstance(Bundle args) {
        SearchVadieoListFragment fragment = new SearchVadieoListFragment();
...
//        getAdapter().setEmptyView(empty_view);
    ...
    public void getData(final boolean refresh, int pageSize, final int pageNum) {
        ShopApi.getProductsList3(keywords,pageNum + "", "1",new ResponseImpl<ShopProductsBean>() {
            @Override
    ...
    public void bindData(BaseViewHolder helper, ShopProductsBean.DataBean.InfoBean item, int position) {
        ...
        ImageUtil.getInstance().loadRoundCornerImage(SearchVadieoListFragment.this,item.getImage_default(),iv_bg,5);
        helper.setText(R.id.tv_title,item.getTitle()+"");
        ImageUtil.getInstance().loadCircleImage(SearchVadieoListFragment.this,item.getLogo(), (ImageView) helper.getView(R.id.iv_shop_logo));
