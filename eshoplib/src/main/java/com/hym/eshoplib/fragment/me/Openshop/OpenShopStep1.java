                    ...
                    case 1:
                        ...
                        CommonApi.uploadFile(_mActivity, files, new ResponseImpl<UploadFilesBean>() {
                            ...
                            public void onSuccess(UploadFilesBean data) {
                                ImageUtil.getInstance().loadRoundCornerImage(OpenShopStep1.this, url, ivUploadImage, 5);
        ...
        setRight_tv("下一步", new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (isEdit && bean != null && bean.getData().getBase().getIs_verify().equals("1")) {
                    DialogUtil.getTowButtonDialog(_mActivity, "提示", "信息修改后需要平台审核您确定要修改么", "取消", "确定", new DialogUtil.OnDialogHandleListener() {
                        @Override
...
//        }
        ...
        //校验全部通过调用接口
        ShopApi.OpenShopStep1(avantar, shopName, category_id,
                region_id, realName, card_id,
                address, phone, ohterName, otherPhone,
                email, attachment_id, new ResponseImpl<Object>() {
                    @Override
    ...
    private boolean EditShop() {
        ...
        String attachment_id = card_1 + "," + card_2 + "," + card_3;
        //校验全部通过调用接口
        ShopApi.EditShop(avantar, shopName, category_id,
                region_id, realName, card_id,
                address, phone, ohterName, otherPhone,
                email, attachment_id, new ResponseImpl<Object>() {
                    @Override
