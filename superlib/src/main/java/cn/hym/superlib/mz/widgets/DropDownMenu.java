...
public class DropDownMenu extends LinearLayout {
    ...
    public DropDownMenu(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        underLine.setLayoutParams(new LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dpTpPx(1.0f)));
        underLine.setBackgroundColor(underlineColor);
        addView(underLine, 1);

        //初始化containerView并将其添加到DropDownMenu
        containerView = new FrameLayout(context);
        containerView.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,
    ...
    public void setDropDownMenu(List<String> tabTexts, List<View> popupViews, View contentView) {
        if (tabTexts.size() != popupViews.size()) {
            throw new IllegalArgumentException("params not match," +
                    " tabTexts.size() should be equal popupViews.size()");
        }
        ...
        popupMenuViews.setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT,
                (int) (DeviceUtils.getScreenSize(getContext()).y * menuHeighPercent)));
        popupMenuViews.setVisibility(GONE);
        containerView.addView(popupMenuViews, 2);

        for (int i = 0; i < popupViews.size(); i++) {
            popupViews.get(i).setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT,
    ...
    private void addTab(List<String> tabTexts, int i) {
        ...
        tab.setLayoutParams(new LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1.0f));
        ...
        if (i < tabTexts.size() - 1) {
            ...
            view.setLayoutParams(new LayoutParams(dpTpPx(0.5f), FrameLayout.LayoutParams.MATCH_PARENT));
